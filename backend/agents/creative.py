...
参数：
- `topic`（必需）：创作主题，简洁概括用户的核心需求
- `context`（重要）：详细的创作上下文信息，需要尽可能全面

**context 必须包含以下维度的详细信息**：

1. **风格调性**：活泼俏皮/专业权威/温馨治愈/幽默搞笑/文艺清新/干货实用
2. **目标人群**：年龄段、性别、职业身份、兴趣爱好等
3. **内容要求**：
   - 必须包含的元素（产品特点、个人经历、数据、案例等）
   - 需要突出的卖点或核心信息
   - 期望的内容深度和专业程度
4. **结构偏好**：
   - 页数要求
   ...
   - 是否需要对比评测
   - 是否需要经验分享形式
5. **关键词和话题**：需要融入的热门话题、关键词、hashtag
6. **情感诉求**：想要激发读者什么样的情感（共鸣、向往、好奇、安心等）
7. **禁忌事项**：不能出现的内容或表达方式

**示例**：
用户说"帮我写一篇关于护肤的小红书，要专业一点，面向25-35岁职场女性，重点推荐早C晚A"

调用：
```json
{{
    "topic": "职场女性护肤攻略-早C晚A详解",
    "context": "【风格调性】专业权威但易懂，避免过于学术化\\n【目标人群】25-35岁职场女性，注重护肤但时间有限，有一定护肤基础\\n【内容要求】\\n- 必须详细讲解早C晚A的原理和正确用法\\n- 包含产品推荐和使用顺序\\n- 需要提供新手入门和进阶版本的方案\\n- 加入常见误区和注意事项\\n【结构偏好】\\n- 6-8页\\n- 包含原理讲解、产品推荐、使用步骤、避坑指南\\n【关键词】早C晚A、抗老、美白、职场护肤\\n【情感诉求】让读者感到专业可信赖，学到干货\\n【禁忌】避免过度营销感，不要夸大功效"
}}
```
